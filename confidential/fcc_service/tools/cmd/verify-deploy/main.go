...
func main() {
	...
	var requestedChecks map[string]bool
	if *checksFilter != "" {
		requestedChecks = make(map[string]bool)
		for _, id := range strings.Split(*checksFilter, ",") {
			if id = strings.TrimSpace(id); id != "" {
				requestedChecks[id] = true
			}
		}
	}
	...
	case "deploy", "register", "all":
		...
		case "register":
			validate.RegisterRegistrationChecksFiltered(report, s.ChainClient, s.Prv, s.TeeExtensionRegistry, s.TeeOwnerAllowlist, *configPath, requestedChecks)
		case "all":
			validate.RegisterDeployChecks(report, s.ChainClient, s.Prv, addresses, *configPath)
			validate.RegisterRegistrationChecksFiltered(report, s.ChainClient, s.Prv, s.TeeExtensionRegistry, s.TeeOwnerAllowlist, *configPath, requestedChecks)
