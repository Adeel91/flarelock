...
**Confidential match payloads** are JSON-encoded `PrivateMatchRequest` values. The encrypted payload is decrypted through the tee-node `/decrypt` endpoint, then the extension reconstructs and verifies the Version 2 FlareLock wallet-signed intents before matching them.
